...
namespace StaffFrontend.test.Proxies.Reviews
{
    ...
    public class GetHiddenReviews
    {
        ...
        {
            Assert.IsTrue(reviews.Where(r => r.hidden).ToList().SequenceEqual(await cpl.GetHiddenReviews(null, null)));
        }

        [TestMethod]
        public async Task ReviewProxy_GetReviews_ItemId()
        {
            Assert.IsTrue(reviews.Where(r => r.productId == 1 && r.hidden).ToList().SequenceEqual(await cpl.GetHiddenReviews(1, null)));
        }

        [TestMethod]
        public async Task ReviewProxy_GetReviews_UserId()
        {
            Assert.IsTrue(reviews.Where(r => r.userId == 1 && r.hidden).ToList().SequenceEqual(await cpl.GetHiddenReviews(null, 1)));
        }
    }
}